...
function HeroGrid() {
  ...
  // Procedural topographic contours — generated once per mount, sized to viewBox 0-100
  const contours = useMemo(() =>
    buildContours({ width: 100, height: 100, levels: 22, seed: 42 })
  , []);

  // Which node is "active" (pulsing stronger) — rotates through list
  const activeIdx = tick % NODES.length;

  return (
    <div className="hero-grid rise-1">
      <div className="hero-inner">
        <div className="hero-lead">
          ...
          <p className="hero-para">
            I turn ambiguous, domain-heavy problems into working systems.
            8+ years across <em>geosciences</em>, <em>AWS data pipelines</em>, <em>GIS</em>,
            and now <strong className="neon-cyan">AI-assisted automation</strong>. Remote-first from Bogotá.
          </p>
        ...
        <div className="grid-window glass-strong">
            ...
            <svg className="grid-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
              ...
              {Array.from({length: 19}, (_, i) => (
                <line key={`v${i}`} x1={(i+1)*5} y1="0" x2={(i+1)*5} y2="100"
                  stroke="oklch(0.55 0.10 300)" strokeOpacity="0.14" strokeWidth="0.12" />
              ))}
              {Array.from({length: 19}, (_, i) => (
                <line key={`h${i}`} x1="0" y1={(i+1)*5} x2="100" y2={(i+1)*5}
                  stroke="oklch(0.55 0.10 300)" strokeOpacity="0.14" strokeWidth="0.12" />
              ))}
              {/* Procedural topographic contours — iso-lines from a value-noise heightfield */}
              ...
              {/* connection lines from Bogotá to each project */}
              {NODES.filter(n => !n.primary).map((n, i) => (
                <line
                  key={n.id}
                  x1="28" y1="58" x2={n.x} y2={n.y}
                  stroke="url(#g-line)"
                  strokeOpacity={activeIdx === NODES.findIndex(x => x.id===n.id) ? 0.9 : 0.25}
                  strokeWidth={activeIdx === NODES.findIndex(x => x.id===n.id) ? 0.35 : 0.15}
              ...
              return (
                <div
                  ...
                  style={{
                    left: `${n.x}%`,
                    top: `${n.y}%`,
                    "--node-color": accentColor[n.accent],
                    "--node-size": `${n.size}px`,
                  }}
